...
class Solver(nn.Module):
    ...
    def train(self, loaders):
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            if (i+1) % args.print_every == 0:
                # save in the history
                training_history['loss_d_latent'].append(d_loss.cpu().detach())
                training_history['loss_d_reference'].append(d_loss.cpu().detach())
                training_history['loss_g_latent'].append(g_loss.cpu().detach())
                training_history['loss_g_reference'].append(g_loss.cpu().detach())
            ...
            if (i+1) % args.show_every == 0:
                ...
                losses_plots_img = args.loss_monitoring_dir + '/losses_' + str(i+1) + '.png'
                fig.savefig(losses_plots_img)

            # generate images for debugging
            if (i+1) % args.sample_every == 0:
                os.makedirs(args.sample_dir, exist_ok=True)
                utils.debug_image(nets_ema, args, inputs=inputs_val, step=i+1, domains=self.domains)
    ...
    def evaluate(self):
        ...
        # TODO: l'ho commentato solo per velocizzare la computazione di un'esecuzione --> ricordati di decommentarlo per tutte le altre esecuzioni
