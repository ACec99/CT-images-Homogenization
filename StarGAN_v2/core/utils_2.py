    ...
    for i, y_trg in enumerate(y_trg_list):
        ...
        for z_trg in z_trg_list:
            ...
            if edge_loss is not None and not x_concat_edge:
                ...
                laplacian_x_fake_all = torch.zeros_like(x_fake)
                laplacian_x_real_all = torch.zeros_like(x_src)
                for b in range(x_fake.shape[0]):
                    loss_edge, laplacian_x, laplacian_y = edge_loss(x_src[b].unsqueeze(0), x_fake[b].unsqueeze(0))
