...
def compute_g_loss(nets, texture_extractor, edge_loss, args, x_real, y_org, y_trg, z_trgs=None, x_refs=None, masks=None):
    ...
    if args.texture_loss:
        ...
        # ---------------------------------------------------------- #
        x_rec_1_ch = x_rec[:, :, start:end, start:end]#.unsqueeze(1)
        x_real_1_ch = x_real[:, :, start:end, start:end]#.unsqueeze(1)
        # ---------------------------------------------------------- #
        ...
        if args.edge_loss:
            ...
            loss_edge, laplace_x, laplace_y = edge_loss(x_real, x_fake)
